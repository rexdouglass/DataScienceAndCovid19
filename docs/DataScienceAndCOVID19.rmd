...
The SARS-COV-2 global pandemic has exposed weaknesses throughout our institutions, and the sciences are no exception. Given the deluge of official statistics and 300+ new COVID-19 working papers posted each day, it is imperative for both consumers and producers of COVID-19 knowledge to be clear on what we do and do not know. In this brief review, we enumerate ways that data science has highlighted these weaknesses and can help address them.^[[@COVID19Primer]]
...
Confirmed case and death counts mechanically depend on testing, but records of tests administered $T_{ct}$ are even worse. In the U.S., much of what we know about trends in testing patterns come from journalistic efforts like the Covid-Tracking Project [@zachliptonCovidTrackingProject2020]. They encountered all of the regular problems plus additional ones specific to ambiguity to what kind of test count is being reported (testing encounters, number of people tested ever, number of swabs tested, etc). The type of test performed (and its false positive and false negative rate) is almost never included as metadata. Nor are the rules about how tests are being rationed and distributed being recorded systematically. Further, these raw counts aren't generally available below the state level.^[See [@NewCOVID19Testing2020] for a rare cross-section.] 

The general failure to track COVID-19 spread directly has led to a proliferation of innovative attempts to use other signals such as web searches, searches of medical databases, social media posts, fevers reported by home thermometer, and traditional flu symptom survailence networks [@koganEarlyWarningApproach2020]. While promising, proxy measures require ground truthing and regular calibration using something like regularly timed serological surveys on smaller geographic sample of the population. [AND SUCH GROUND TRUTH DOESN'T EXIST, RIGHT?]
...
The necessity of directly including the generating process of the testing data in models of the epidemiological data generating process can't be understated. Per-capita testing rates are so temporarily correlated with per-capita case rates they are more of a proxy of testing availability than actual cases [@kaashoekCOVID19PositiveCases2020a]. Spatially, per-capita testing rates also correlate with urbanity and co-morbids [@souchCommentaryRuralUrban2020]. How many tests are given and to who varies systematically in response to conditions on the ground with both periods of rationing and blitzes.

We largely do not have exogenous assignment of interventions. Governments implemented policies in response to both local conditions and national signals. Populations responded independently of government orders, both reducing their activity prior to being ordered to and also increasing their activity prior to being officially allowed to. Governments, the public, and the disease are all responding simultaneously to each other in often nonlinear and observed ways. Statistical instruments that cause a government intervention but does not cause [ARE INDEPENDENT OF?] either testing or cases directly except through the government intervention are few and far between. Further, interventions are often implemented simultaneously or in a rolling cumulative pattern directly in response to changes in cases and testing results, making isolating the effect of any one treatment exceptionally difficult. 

Even if we had an exogenous intervention, its treatement effect is still unlikely to be identified since almost any intervention will affect both cases and testing. Estimating an effect requires a combination of sharp constraints on some parameters and informative priors on the relationship between the number of tests and the number of cases [@kubinecRetrospectiveBayesianModel2020].


# Evaluation and Interpretation

One of the most promising modeling developments in the early pandemic was the rigorous uniform forecast evaluations [@nicholasgreichReichlabCovid19forecasthubPrepublication2020]. COVID-19 models serve both a short term forecasting role needed for making policy decisions, and also the goal of identifying the actual underlying data generating process, i.e. how COVID spreads, for long term planning and response. In practice, many forecasting groups have focused almost entirely on the former, constantly tweaking their model to fit recent in-sample data without any theoretical or empirical justification. Notoriously, many early simple growth models fit to the early takeoff period of infections performed well right up until the curve broke and then failed entirely. A parade of predicted peaks in cases since continue the tradition, with groups celebrating success on uninteresting, short-term auto-correlations and ignoring failures on actually interesting unexpected shifts in trends. The only solution is to have a very long memory and to hold models accountable for their out-of-sample performance on unseen future data. [IS THIS WHAT THE UNIFORM FORECAST EVALUATIONS DO? WHAT'S THE ENFORCEMENT?]

Other developments in observational COVID-19 work are mush less promising.^[We omit citations falling under the criticisms provided below as they are working papers and likely to change before finalization.] An embarassment [OVERABUNDANCE?] of observational work still presents correlations as evidence of causation. Without identification, correlations on short highly autocorrelated time series are as likely to be misleading as informative. The SEIR model expects a nonlinear and highly auto-correlated pattern of an increasing infection rate that then levels off independent of any interventions. An unscrupulous analyst could intentionally find that certain interventions increased (or decreased) spread by selecting cases where those interventions were implemented early (or late), completely independent of the intervention's actual effect. 

In another bad habit common in the social sciences, statistically distinguishable correlations are being pursued for their own sake instead of attempting to explain variation in COVID-19 outcomes. Papers that can show a particular political party or demographic group is 'worse' on some COVID-19 dimension receive the most acclaim and attention. Despite different arrival times and different rates of testing in rural and poorer areas, a later takeoff date for infections in some states is presented as evidence of incompetence or harmful ideology. Such results lack strong explanatory power, large substantive effects, or clearly policy recommendations; they are great for making headlines but do little to help us end the current pandemic. 

Perhaps the most egrious trend in recent scholarship is setting up straw man null hypothesis and then presenting the inability to reject them as positive evidence for medical and safety decisions, e.g. social distancing might not be required because certain mass events were not immediately followed by a large uptick in cases. It is scientific malfeasance to ever present null results as evidence for any health and safety claim on COVID-19; we do not have sufficient measurement, model development, causal identification, or model performance justify makeing potentially life-or-death conclusions. Confidence intervals in this space should be interpreted as the lower bound of uncertainty, not the upper or even the most likely bound for any real world observational research on COVID-19 being presented today.

# Conclusion

This necessarily brief review omitted positive developments in studying COVID-19 outside of macro-observational settings. There has been remarkable progress in areas of diagnosis, clinical treatment, gnomic tracking, and molecular tracking. Data science has contributed to the rapid collaboration, development, and dissemination of research in a way not seen in prior disease outbreaks. Further, our review is overly U.S. centric, with other countries like South Korea monitoring the disease so effectively they succeeded at containment without having to resort to difficult mitigation. We also neglected topics like tracing, and the acompaning contributions from the tech field such as monitoring through mobile apps and social media.

Any research related to COVID requires healthy caution of and respect for how little we actually know about the history of this pandemic. Practioners working on these questions and with these data will be deeply familiar with many of these concerns, but some may be especially subtle or less prominant  within one's main field of study. At a minimum, there is research being produced today which ignores much of these known methodological problems and subsequently generates confusion for novice consumers of analysis. We hope this enumeration of challenges in measurement, inference, and evaluation, can help both consumers and producers alike be part of the solution. 


[LEFTOVER?] Bayesian estimate of R_t @[#BayesianNowcastingAdjustment]
